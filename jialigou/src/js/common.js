...
define(function(){
	...
	return {
		...
		randomColor : function(){
			...
			// 使用其它封装
			var r = this.randomNumber(10,255);
			var g = this.randomNumber(10,255);
			var b = this.randomNumber(10,255);
		 ...
		 */
		hasLogin: function(){
			var cuUsername = this.Cookie.get('username');
		    if(cuUsername){
		        var str = 'Hi,' + cuUsername + ' 欢迎回嘉!';
		        $('.showUsername').text(str).append(($('<a>').prop('href','./login.html')).css({margin:'10px'}).text('[退出]').prop('class','logout'));

		    }else{
		        var loginBtn = $('<a>').prop('href','./login.html').text("[登陆]").css({margin:'5px'});
		        var logoutBtn = $('<a>').prop('href','./register.html').text("[免费注册]");
		        $('.showUsername').append($('<li>').append(loginBtn).append(logoutBtn));
		    }
		    $('.logout').click(function(){
		        common.Cookie.remove('username');
		    })
		},

		addShoppingCar : function(proid,qty){
			var allBuy = this.Cookie.get('allBuy')
			if(allBuy == ''){
				var allBuy = [];
			}else{
				var allBuy = JSON.parse(allBuy)
			}
			if(allBuy.length > 0){
                for(var i = 0; i < allBuy.length ; i++){
                    if(proid == allBuy[i].proid){
                        allBuy[i].goodQty = Number(allBuy[i].goodQty) + qty;
                        break;
                    }
                    if(i == allBuy.length - 1){
                        allBuy.unshift({
                            proid : proid,
                            goodQty : qty
                        })
            ...
            this.Cookie.set('allBuy',JSON.stringify(allBuy));
		},
		...
		//判断，改变cookie中的allBuy,并重新存入cookie

		/**
		 * [点击进入相应列表页]
		 * @opt  {Any} 容器 [传入的数据]
		 * 例如： common.locationTo('.goods') 中，点击.goods下的所有a会跳到a的内容的列表页
		 */
		locationTo : function(opt){
			$(opt).click(function(event){
		        var target = $(event.target);
		        if(target.is('a') && target.text() && !target.prop('href')){
		            location.href = '/src/goods_list.html' + '?sort=' + encodeURI(target.text());
		        }
		    })
